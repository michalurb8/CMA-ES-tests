...
def evaluate(repair_mode: str, dimensions: int = 10, iterations: int = 10, objectives: List = None, lambda_arg: int = None, visual: bool = False):
    ...
    if objectives is None:
            # objectives = ['quadratic', 'felli', 'bent', 'rastrigin', 'rosenbrock', 'ackley'] # CHOOSE ANY SUBSET OF THIS LIST
            objectives = ['felli']
    ecdfs_list = []
    sigmas_list = []
    diffs_list = []
    ...
    return (formatted_ecdfs, formatted_sigmas, formatted_diffs, formatted_eigens)
...
def run_test(dimensions: int = 10, iterations: int = 10, lbd: int = None, visual: bool = False):
    ...
    for rmode, v in runsc:
        ecdf, sigma, diff, eigen = evaluate(rmode, dimensions, iterations, None, lbd, visual and v)
